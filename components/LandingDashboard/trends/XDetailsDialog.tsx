...
interface XDetailsDialogProps {
  trend: TrendWithStats | null;
  isOpen: boolean;
  onClose: () => void;
}

const XDetailsDialog: React.FC<XDetailsDialogProps> = ({
  trend,
  isOpen,
  onClose,
}) => {
  ...
  // Mock data for X (Twitter) style chart
  const generateTrendsData = () => {
    const days = [];
    const baseVolume = trend.tweet_volume || 50000;
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      if (i < 7) trendMultiplier = 1.25; // Recent spike
      if (isWeekend) trendMultiplier *= 0.9; // Slightly lower on weekends
      ...
      days.push({
        ...
        tweets: Math.round(baseVolume * (normalizedValue / 100)),
      });
    }
    return days;
  };
  ...
  interface XTooltipProps {
    active?: boolean;
    payload?: Array<{ value: number; payload: { fullDate: string; value: number; tweets: number } }>;
    label?: string;
  }

  const XTooltip = ({ active, payload }: XTooltipProps) => {
      ...
      return (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-3 shadow-lg">
          ...
          <p className="text-gray-900 dark:text-white font-semibold">
            Tweet Activity: {data.value}
          </p>
          <p className="text-gray-600 dark:text-gray-400 text-xs">
            ~{formatTweetVolume(data.tweets)} tweets
          </p>
        </div>
      );
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[95vw] h-[90vh] max-w-md bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 p-0 rounded-lg md:w-[90vw] md:h-[85vh] md:max-w-5xl md:border md:border-gray-200 dark:md:border-gray-800 overflow-hidden flex flex-col">
        ...
        <DialogHeader className="px-3 py-2 md:px-5 md:py-3 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 flex-shrink-0">
          <DialogTitle className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1.5 sm:gap-2">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3">
              <div className="flex items-center gap-1.5">
                <Twitter className="w-4 h-4 md:w-5 md:h-5 text-gray-700 dark:text-gray-300 flex-shrink-0" />
                <span className="text-sm md:text-xl font-normal text-gray-900 dark:text-white break-words line-clamp-2">
                  {trend.name}
                </span>
              </div>
              <Badge className="bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-300 border-gray-200 dark:border-gray-700 w-fit text-xs">
                Trending #{trend.rank}
              </Badge>
            </div>
          </DialogTitle>
                ...
                <Card className="border-gray-200 dark:border-gray-800">
                  <CardContent className="p-2 md:p-4">
                    ...
                    <div className="mt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs text-gray-600 dark:text-gray-400">
                      <span className="text-left line-clamp-2">Numbers represent tweet activity relative to the highest point.</span>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2 text-xs">
                        <span className={`px-1.5 py-0.5 rounded text-xs ${
                          ...
                          {trendDirection > 0 ? 'Trending Up' : 'Trending Down'}
                        </span>
                      </div>
                    </div>
                  </CardContent>
                ...
                <CardContent className="space-y-3 md:space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 md:gap-4">
                    <div className="space-y-1.5 md:space-y-2">
                      <h4 className="font-medium text-xs text-gray-700 dark:text-gray-300">Tweet Details</h4>
                      <div className="space-y-1 text-xs">
                        ...
                        <div className="flex justify-between gap-2">
                          <span className="text-gray-600 dark:text-gray-400">Average Activity:</span>
                  ...
                  <div className="pt-2.5 md:pt-3 border-t border-gray-200 dark:border-gray-700">
                    <a
                      ...
                      className="inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-gray-900 hover:bg-gray-800 dark:bg-gray-100 dark:hover:bg-gray-200 text-white dark:text-gray-900 rounded-lg transition-colors font-medium text-xs w-full sm:w-auto"
                    >
