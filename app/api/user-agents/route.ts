...
const generateMockCreditsData = () => {
  ...
  return {
    ...
    last_expense_reason: expenseReasons[Math.floor(Math.random() * expenseReasons.length)],
    last_expense_date: Math.floor(Date.now() / 1000) - Math.floor(Math.random() * 30 * 24 * 60 * 60), // Random within last 30 days
    last_expense_type: expenseTypes[Math.floor(Math.random() * expenseTypes.length)]
  };
};
